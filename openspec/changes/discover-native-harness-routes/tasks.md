...
- [x] 5.3 Run `npm run doctor`, `npm run smoke:release` only in their existing zero-model/read-only modes, and `openspec validate discover-native-harness-routes --strict`; record unavailable external Harnesses as bounded diagnostics rather than repair them.
