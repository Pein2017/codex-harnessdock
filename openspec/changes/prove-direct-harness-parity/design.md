...
The accepted explicit-effort plus provenance-v3 base permitted implementation of generic matrix infrastructure and supported Pi/OpenCode rows before Claude discovery. The separately authorized zero-prompt Agent SDK initialization witness now closes Claude Code 2.1.250 exact dynamic model/effort discovery: its exact-route inventory cell records the complete exact model/effort set in a sanitized source-bound receipt, with no static catalog/config/authority/transport fallback. Deterministic Claude rows remain independently evidenced, and the matrix is promotion-eligible at 32 `pass`, 0 `fail`, 0 `hold`, and 10 capability-derived `not_applicable` cells.
...
| `same_session_recovery_prompt` | capability `not_applicable` until supported | fixture: same `S`, exactly one generated new recovery prompt, no original-input duplicate, distinct provider message/result identities | capability `not_applicable` until supported |
...
`same_session_recovery_prompt` is the sole row for Claude's `automaticRecovery=same_session_recovery_prompt`. It proves a fresh process resumes `S` and sends exactly one generated recovery prompt as distinct new input, with no original-input duplicate and distinct provider-native user/result identities when the fixture exposes them. It does not prove recovery of interrupted `T1`; the exact-session-transport cell is `not_applicable`.
...
1. Start on the accepted explicit-effort plus provenance-v3 base. Record the Claude Code 2.1.250 exact dynamic model/effort inventory from the separately authorized zero-prompt Agent SDK initialization witness; do not substitute static discovery.
...
3. Add fresh-process `exact_session_continuation` tests, separate no-prompt old-turn observation/reconciliation tests, exact-session-transport tests only for routes claiming `automaticRecovery=exact_session_transport`, same-session recovery-prompt tests for routes claiming `automaticRecovery=same_session_recovery_prompt`, plus explicit capability-derived `not_applicable` evidence.
