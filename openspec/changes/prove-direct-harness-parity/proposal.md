...
- Add a source-independent differential acceptance matrix for direct native invocation versus the corresponding HarnessDock Driver, with one fixture family per Harness and explicit capability-based `not_applicable` cells.
...
- Depend on the other three changes and on synchronization or archival of `discover-native-harness-routes`; this change is an acceptance gate, not another runtime plane.
