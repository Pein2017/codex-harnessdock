...
- [x] 6.5 Reproduce and fix capability-schema v2/v3 Agents missing the later `nativeProgress` dimension so they remain read-only compatible and cannot poison an unrelated fresh v4 spawn; preserve strict v4 validation and byte-for-byte historical state.
- [x] 6.6 Run focused Driver/migration/public-spawn tests, replay the exact retained production route read-only, then rerun strict OpenSpec validation, `git diff --check`, and `npm run check`.
