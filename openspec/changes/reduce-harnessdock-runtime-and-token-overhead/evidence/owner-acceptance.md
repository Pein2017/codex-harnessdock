...
### Fresh installed cold-start acceptance (Task 6.4)
...
The remaining live boundary was closed after the exact managed service reached its real one-hour idle threshold. Thirty-eight lease files were first proven to be test-fixture residue (`composed-root`, `spawn-admission-root`, or bounded `failures-root-*` lineage), removed exactly, and prevented from recurring by giving the three owning focused suites a disposable per-file `CODEX_HOME`; the same 31 tests then passed without changing the production lease-set hash. The first real reap attempts exposed two Linux-only blockers rather than being waived: the reaper's own health keep-alive was observed as a peer, and `/proc/net/tcp6` used the valid `remote_address` header. Focused lifecycle regressions were added, the peer check was moved before the reaper-owned health request, the exact IPv6 header was admitted, and the original owned PID `3948641` then reaped with `{reaped:true, reason:"terminated"}`. No Agent/session/history state was deleted.

A fresh isolated Codex task (`01a04cc7-be42-7332-9d4f-2c522f12f59d`) explicitly loaded the installed `0.24.1+codex.20260829090851` bootstrap so Desktop's already-resident old MCP processes were not mistaken for a fresh load. Its first `list_harnesses` observed OpenCode `ready`, detail `dormant_native_config`, `liveValidated=false`, exact Luna/Sol/Terra routes, and efforts `high/low/max/medium/none/xhigh`; Claude and Pi remained `ready` and `liveValidated=true`. The production service manager then returned `{status:"managed"}` from an absent endpoint, and the second installed Plugin call observed OpenCode `ready`, `liveValidated=true` with the same routes while Claude/Pi stayed ready. The exact eight tools remained visible. This cold-start sent no Harness model request; the one authorized Luna-low Harness turn remains the sole Task 6.3 model turn. The acceptance-created service had zero leases and peers and its exact PID was terminated as test teardown.
