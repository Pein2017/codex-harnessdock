...
An untargeted call observes current-root completion; a targeted call observes fixed turns. The hidden upper bound is 3600000 ms. Set `wake_on_progress: true` only for exactly one target and an intermediate update; progress returns at most one sanitized update and hook activity stays private. Completion has priority with complete stored `completion_message` and token; acknowledge each consumed `delivery_token` once later. Unrelated root activity remains available to its proper consumer. A timeout means no eligible completion was visible, not failure: do not repeat progress waiting, narrate unchanged timeouts, or call `list_agents` or `read_agent_messages` immediately afterward merely to recheck completion; call `wait_agent` again directly. Wait cannot reactivate an ended Codex turn.
